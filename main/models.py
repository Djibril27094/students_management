...
class Teacher(models.Model):
    ...
    profile_img=models.ImageField(upload_to='profile_teacher_imgs/',null=True)
    ...
    def tatal_teacher_students(self):
        total_student = StudentCourseEnrollment.objects.filter(course__teacher=self).count()
...
class CoursCategory(models.Model):    
    ...
    class Meta:
        verbose_name_plural = "2. Cours Categories"
    
    # Total couses category
...
class Course(models.Model):
    category = models.ForeignKey(CoursCategory , on_delete=models.CASCADE)
...
class Student(models.Model):
    ...
    profile_img = models.ImageField(upload_to='profile_student_imgs',null=True)
    verify_status=models.BooleanField(default=False)
    otp_digit=models.CharField(max_length=10,null=True)
    login_via_otp=models.BooleanField(default=False)
    ...
    def complete_assignment(self):
        complete_assignments = StudentAssignement.objects.filter(student=self,student_status=True).count()
        return complete_assignments
    
    def pending_assignment(self):
        pending_assignments = StudentAssignement.objects.filter(student=self,student_status=False).count()
...
class CourseRating(models.Model):
    ...
    class Meta:
        verbose_name_plural="8. Course Raitings"
    
    def __str__(self):
        return f"{self.course}-{self.student}-{self.rating}"

# Courses Assignment and reviews
class StudentAssignement(models.Model):
    ...
    class Meta:
        verbose_name_plural="9. Course Assignment"
